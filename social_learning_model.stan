  ...
  for(replicate in 1:n_replicates) {
    ...
    for(trial in 1:n_trials) {
      ...
      for(agent_order_number in 1:n_agents) {
        ...
        target += categorical_lpmf(action | actions_probabilites);
        //action ~ categorical(actions_probabilites); Alternative: With the ~ notation, normalizing constants are dropped, and the lp__ parameter is correct up to that constant and useless for model comparison. This notation is faster only with likelihoods with lots of normalizing constants.
